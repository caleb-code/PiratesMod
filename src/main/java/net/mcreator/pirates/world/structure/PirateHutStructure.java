...
package net.mcreator.pirates.world.structure;
...
public class PirateHutStructure extends PiratesElements.ModElement {
	public PirateHutStructure(PiratesElements instance) {
		super(instance, 7);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 7500) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("pirates", "piratehut"));
						if (template == null)
							return false;
